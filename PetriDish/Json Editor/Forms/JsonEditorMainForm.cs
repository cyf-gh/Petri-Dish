...
namespace ZTn.Json.Editor.Forms {
    public sealed partial class JsonEditorMainForm : Form {
        ...
        public void OpenWithFile( string filePath ) {
            using( Stream stream = File.OpenRead( filePath ) ) {
                SetJsonSourceStream( stream, filePath );
            }
        }

        public void OpenWithStream( Stream stream, string filePath ) {
            SetJsonSourceStream( stream, filePath );
        }
        ...
        private void JsonEditorMainForm_FormClosing( Object sender, FormClosingEventArgs e ) {
            DialogResult result = MessageBox.Show( "Save the modified file?", "Info", MessageBoxButtons.OKCancel, MessageBoxIcon.Information );
